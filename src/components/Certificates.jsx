...
const Certificates = () => {
  ...
  return (
    <section id="certificates" className="bg-[#f59e0b] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans">
      ...
      <div className="max-w-4xl mx-auto relative z-20">
        ...
        <div data-aos="fade-up" className="mb-16 text-center">
          ...
          <p className="text-amber-950 text-base md:text-lg font-bold max-w-lg mx-auto">
            Professional qualifications demonstrating specialized technical expertise.
          </p>
        </div>

        {/* Certificate Cards Grid (2-column for exactly 2 certificates) */}
          ...
          {certificates.featured.map((cert, index) => (
            ...
            >
              <div>
                ...
                <h3 className="text-white text-2xl font-black mb-1 tracking-tight group-hover:text-amber-400 transition-colors duration-300">
                  {cert.name}
                </h3>
              ...
              <div className="pt-4 border-t border-white/10 flex items-center gap-2 text-white/50 group-hover:text-white transition-colors">
                ...
                <span className="text-xs font-bold uppercase tracking-wider font-mono">Open Certificate Preview</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeCert && (
        ...
        >
          {/* Modal Container */}
          <div 
            className="bg-zinc-900 border border-white/10 max-w-4xl w-full rounded-3xl p-6 relative flex flex-col items-center shadow-[0_30px_60px_rgba(0,0,0,0.8)]"
          ...
          >
            ...
            {/* Cert Title */}
            <h3 className="text-white text-xl md:text-2xl font-black mb-1 mt-4 text-center pr-10">
              {activeCert.name}
            </h3>
            <p className="text-[#f59e0b] text-sm font-mono font-bold tracking-widest uppercase mb-6">
              {activeCert.issuer}
            </p>

            {/* Image Preview */}
            <div className="w-full max-h-[60vh] overflow-hidden rounded-xl bg-black border border-white/5 flex justify-center items-center shadow-inner mb-4">
              <img 
                src={activeCert.img} 
                alt={`${activeCert.name} Certificate`} 
                className="max-w-full max-h-[60vh] object-contain hover:scale-105 transition-transform duration-500"
              />
            </div>
            
            <p className="text-white/60 text-xs md:text-sm text-center max-w-lg leading-relaxed font-medium mt-2">
              {activeCert.desc}
            </p>
          </div>
        </div>
