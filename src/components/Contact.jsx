...
const Contact = () => {
  ...
  const buildGmailComposeLink = (subject = '', body = '') => {
    const params = new URLSearchParams({
      view: 'cm',
      fs: '1',
      to: personalInfo.emails.primary,
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isConfigured) {
      ...
      const mailtoLink = buildGmailComposeLink(
  ...
  return (
    <section ref={ref} id="contact" className="bg-[#0a0a0a] w-full min-h-screen relative overflow-hidden flex items-end pt-32 pb-0 md:pb-0 border-t border-gray-900">
      ...
      <div className="relative z-10 w-full flex justify-end items-end">
        <div
          data-aos="fade-up"
          className="bg-[#f59e0b] w-full md:w-[85%] lg:w-[75%] p-8 md:p-16 text-white flex flex-col justify-between"
        >
              ...
              <div className="flex-1 flex flex-col gap-8 text-xs text-white/70 font-medium">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6">
                  <p className="max-w-[250px] leading-relaxed">
                    For urgent inquiries, reach me at <a href={buildGmailComposeLink('Urgent Inquiry', '')} className="underline hover:text-white transition-colors">{personalInfo.emails.primary}</a>
                  </p>

                  <button
                    type="submit"
                    disabled={status === 'sending'}
                    className={`px-8 py-3 rounded-full border border-white/40 text-white font-bold flex items-center justify-center gap-3 transition-all duration-300 group whitespace-nowrap self-start sm:self-auto ${status === 'sending'
                        ? 'opacity-50 cursor-not-allowed bg-white/10'
                        : status === 'success'
                          ? 'bg-green-600 border-green-500 text-white shadow-[0_0_20px_rgba(22,163,74,0.4)]'
                          : status === 'error'
                            ? 'bg-amber-800 border-red-700 text-white'
                            : 'hover:bg-white hover:text-[#f59e0b]'
                      }`}
                  >
                    {status === 'sending' ? (
