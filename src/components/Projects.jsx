...
const ProjectCard = ({ project, aosDelay }) => (
  <div 
    ...
    className={`relative rounded-2xl p-[1px] group transition-all duration-500 ${
      project.isFlagship 
        ? 'bg-gradient-to-br from-amber-500/50 via-white/10 to-amber-500/30 hover:from-amber-500 hover:via-amber-400/30 hover:to-amber-500/60' 
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      {/* Badge */}
      {project.badge && (
        <span className="inline-block text-xs font-bold tracking-widest uppercase text-amber-400 bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20 mb-4">
          {project.badge}
        </span>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {project.techTags.map((tag) => (
          <span 
            key={tag}
            className="px-3 py-1 text-xs font-bold text-white/70 bg-white/5 rounded-full border border-white/10 hover:bg-amber-500/20 hover:border-amber-500/30 hover:text-amber-300 transition-all duration-300 cursor-default"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {project.links.demo !== undefined && (
          <a 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              project.links.demo 
                ? 'bg-[#f59e0b] text-white hover:bg-amber-600 hover:shadow-[0_0_20px_rgba(245,158,11,0.4)]' 
        ...
        {project.links.frontendDemo && (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#f59e0b] text-white text-sm font-semibold hover:bg-amber-600 hover:shadow-[0_0_20px_rgba(245,158,11,0.4)] transition-all duration-300"
          >
            <ExternalLinkIcon />
            Frontend Demo
          </a>
